...
export default function example() {
	...
	const floorBody = new CANNON.Body({
		mass: 0, //바닥은 움직이지 않게 (중력에 영향받지 않게) 
	...
	const boxShape = new CANNON.Box(new CANNON.Vec3(0.5, 0.5, 0.5)); //박스에 물리엔진을 적용하기 위한 box 중심으로 부터 떨어진거리
	...
	function draw() { //draw 함수에서 물리엔진을 위해 만든 물체들을 연결시킴
		...
		floorBody.position.copy(floorBody.position);
		boxMesh.position.copy(boxBody.position); //boxMesh 의 위치를 boxBody의 위치를 복사 그러므로 위에 있는 boxMesh의 위치가 무시된다
